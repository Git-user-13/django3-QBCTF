...
def updateHint(request,pk):
    ...
    return render(request, 'updateHint.html', {'form':form,'hint':hint})
...
def questDetail(request,pk):
    ...
    return render(request, 'questDetail.html',{'eachProduct':Product.objects.get(id=pk),'hints':Flags.objects.all(),'completeds':complete.objects.all()})
...
def addFlag(request,pk):
        ...
        if value == '0':
            ...
            if form.is_valid():
                ...
                if (hints.f1.lower() == flag.lower()):
                    ...
                    return render(request, 'addFlag.html',{'hints':Flags.objects.get(id=pk),'completeds':complete.objects.all(),'value':value,'error':error,'token':token,'form':form})
            else:
                form = AnswerForm()
        else:
            return redirect('quest',pk = var)
    ...
    return render(request, 'addFlag.html',{'hints':Flags.objects.get(id=pk),'completeds':complete.objects.all(),'value':value,'form':form,'error':error,'token':token})
...
def finish(request,pk):
    ...
    return render(request,'finish.html',{'completeds':complete.objects.filter(hint_id = pk)})
...
def details(request, pk):
    ...
    return render(request, 'details.html',{'board':board,'hints':hints,'products':products,'completeds':completeds,'attempts':attempts})
